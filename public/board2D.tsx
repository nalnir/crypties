...
export const Board2D = () => {
    ...
    return (
        <div className="flex-col justify-between h-screen p-3 bg-primary-400">
            <div className="flex flex-col justify-between h-full">

                <div className="grid items-start grid-cols-5 gap-x-3">
                    {enemyCardsState.enemyCardsOnTheBoard.map((card, index) =>
                        <EnemyCard
                            key={index}
                            cardIndex={index}
                            card={card}
                ...
                <div className="grid items-start grid-cols-5 gap-x-3">
                    {playerCardsState.playerCardsOnTheBoard.map((card, index) =>
                        <PlayerCard
                            key={card.id}
                            cardIndex={index}
                            card={card}
