...
export const cardCreationRouter = createTRPCRouter({
    uploadMetadataToIPFS: authenticatedProcedure
        .input(original_card)
        .mutation(async (opts) => {
    ...
    uploadMetadataToS3: authenticatedProcedure
        .input(z.object({
            original_card,
            tokenId: z.number()
        }))
        .mutation(async (opts) => {
            ...
            return 'uploadMetadataToS3'
        }),

    getTokenId: authenticatedProcedure
        .input(z.object({
            generation: z.number()
        }))
        .query(async (opts) => {
    ...
    bumpTokenId: authenticatedProcedure
        .input(z.object({
            generation: z.number(),
        }))
        .mutation(async (opts) => {
    ...
    mintBulk: authenticatedProcedure
        .input(z.object({
        ...
        .mutation(async (opts) => {
            const inputs = opts.input;
            const max_tokens_to_mint = env.BULK_MINT_MAX ?? '50';
    ...
    getCurrentGeneration: authenticatedProcedure
        .query(async () => {
    ...
    getCurrentCardId: authenticatedProcedure
        .query(async () => {
