    ...
    const handleCreate = async () => {
        try {
            if (user?.walletAddress) {
                ...
                progressiveLoaderActions.openProgressiveLoader(11, { position: 1, description: 'Checking if player used the creation power' })
                ...
                if (!resIsPlayerClass) {
                    return handleError('Somethign went wrong with the ChatGPT check')
                } else {
                    if (resIsPlayerClass.toLowerCase().includes('false')) {
                        ...
                        progressiveLoaderActions.setActiveStep({ position: 9, description: `Generating possible items used by ${correctedName} class` })
                        const playerClassItem = await generateDescription.mutateAsync({
                            prompt: `${correctedName} item. ONE WORD`
                        })

                        console.log('playerClassItem: ', playerClassItem)

                        progressiveLoaderActions.setActiveStep({ position: 10, description: 'Generating images' })
                        const images = await generateFantasyPlayerClassImages.mutateAsync({
                            prompt: `fantasy ${playerClassItem}, item, center shoot, everything visible, magical background, NO PEOPLE`,
                        ...
                        progressiveLoaderActions.setActiveStep({ position: 11, description: 'Bumping creation power tries' })
